# COMPLEX SOLUTIONS S.L. — Data Analytics / BI

Base de un proyecto de portfolio para analizar ventas, rentabilidad, clientes, inventario y pipeline comercial de una distribuidora tecnológica española. Esta primera fase contiene exclusivamente la estructura PostgreSQL; no crea datos históricos ni una aplicación ERP/CRM.
...
Cuando el servicio aparezca como `healthy`, el esquema queda inicializado automáticamente desde `database/schema.sql`. Se crean 14 tablas: `employees`, `suppliers`, `categories`, `customers`, `products`, `sales`, `sale_items`, `purchase_orders`, `purchase_items`, `inventory_movements`, `leads`, `opportunities`, `crm_activities` e `invoices`.

Para comprobar la estructura y restricciones:

```bash
docker compose exec -T postgres sh -c 'psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -f /tests/schema_integrity.sql'
```

El script ejecuta una cadena mínima de relaciones y verifica que se rechacen cantidad cero y probabilidad fuera del rango permitido; finaliza con `ROLLBACK`, por lo que no deja datos de prueba.

Para reconstruir una instalación local vacía después de modificar `database/schema.sql`:
...
> `down -v` elimina el volumen local de PostgreSQL y, con él, todos los datos locales. No lo uses si quieres conservarlos.
